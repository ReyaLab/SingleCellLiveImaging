...
for j = 1:length(experiments)
    load(fullfile(outputfolder,[experiments{j},' final.mat']))
    numCells = length(int_tracks_osteovascular);
    for k = 1:numCells
        cells(counter).expt = experiments{j};
        cells(counter).ID = k;
        cells(counter).frames = int_tracks_osteovascular{k}(:,1);
        cells(counter).times = int_tracks_osteovascular{k}(:,2);
        cells(counter).x = int_tracks_osteovascular{k}(:,3);
        cells(counter).y = int_tracks_osteovascular{k}(:,4);
        cells(counter).disp = int_tracks_osteovascular{k}(:,5);
        cells(counter).velocity = int_tracks_osteovascular{k}(:,6);
        cells(counter).osteoDist = int_tracks_osteovascular{k}(:,7);
        cells(counter).vascularDist = int_tracks_osteovascular{k}(:,8);
        if j == 1;
...
save(fullfile(outputfolder,'allCells'),'cells')    
